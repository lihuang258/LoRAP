...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        ##稍微做了修改，使其也能接收safetensors格式的权重文件
        if os.path.exists(os.path.join(path, WEIGHTS_NAME)):
            ...
            WEIGHTS_NAME2 = WEIGHTS_NAME.replace(".bin", ".safetensors")
            if os.path.exists(os.path.join(path, WEIGHTS_NAME2)):
                filename = os.path.join(path, WEIGHTS_NAME2)
                adapters_weights = load_file(filename=filename,device="cuda" if torch.cuda.is_available() else "cpu")
                adapters_weights["state_dict"] = adapters_weights

                print("Load from adapter:", WEIGHTS_NAME2)
            else:
                try:
